...
MissionComplete(){
	Loop
	{
		firstimage:= SearchButtonAndClick("PlayAgain_1.png") 
		if(firstimage = true){
			LogMessage("Play Again button available.")
			Break
		} 
		secondimage:= SearchShortButtonAndClick("MissionComplete_1.png") 
		if(secondimage = true){
			LogMessage("Mission completed. OK") 
		} 
		Sleep 3000
	}  
}
...
SearchShortButtonAndClick(path){
	ImageSearch, FoundX, FoundY, 0, 0, A_ScreenWidth, A_ScreenHeight, %path%
	if (ErrorLevel = 2){
		LogMessage("Could not conduct the search")
		return false
